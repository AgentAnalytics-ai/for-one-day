...
export function UnsentMessagesBox() {
  ...
  const handleCreateMessage = async (e: React.FormEvent) => {
    ...
    if (!formData.child_name || !formData.message_content) {
      toast.error('Please fill in child name and message')
      return
    }
    ...
    try {
      ...
      const account = emailAccounts.find(a => a.child_name === formData.child_name)

      const { error } = await supabase
        .from('unsent_messages')
        .insert({
          user_id: user.id,
          child_email_account_id: account?.id || null,
          child_name: formData.child_name,
          child_photo_url: photoUrl,
          message_content: formData.message_content,
          message_title: formData.message_title || `A Letter for ${formData.child_name}`,
          status: 'draft'
        })
      ...
      {messages.length === 0 && !showCreateForm ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {messages.map((message) => {
            const account = emailAccounts.find(a => a.id === message.child_email_account_id)
              ...
              >
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                  <p className="text-xs text-blue-800 font-medium">
                    ✉️ This will be sent to {hasEmail ? account?.email_address : 'their email (create account in Settings)'}
                  </p>
                </div>
